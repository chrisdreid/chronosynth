...
class RawFormatOutput:
    """Converts internal data structure to raw timeslots format."""
    
    @staticmethod
    def format(data: Dict[str, Any], field_config: Dict[str, Dict[str, Any]], normalize: bool = False) -> Dict[str, Any]:
        """
        Format data to raw timeslots format.
        ...
        for field, config in field_config.items():
            result["fields"][field] = {
                ...
                "min": config["min"],
                "max": config["max"],
                "mean": config.get("mean", (config["min"] + config["max"]) / 2)
            }
            ...
            # Add noise configuration if available
            if "noise_amount" in config:
                noise_range = (config["max"] - config["min"]) * 0.01 * config["noise_amount"]
                result["fields"][field]["noise_min"] = -noise_range
                result["fields"][field]["noise_max"] = noise_range
                result["fields"][field]["noise_mean"] = 0
        ...
        for item_name, item_data in data["items"].items():
            ...
            for field, values in item_data.items():
                if field in field_config:
                    result["data"][item_name]["timeseries"][field] = {}
                    field_min = field_config[field]["min"]
                    field_max = field_config[field]["max"]
                    field_range = field_max - field_min
                    
                    for i, timestamp in enumerate(data["timestamps"]):
                        ts_str = RawFormatOutput._format_timestamp(timestamp)
                        
                        # Apply normalization if requested
                        if normalize and field_range > 0:
                            if HAVE_NUMPY and isinstance(values, np.ndarray):
                                # Use numpy for normalization if available
                                norm_value = (values[i] - field_min) / field_range
                                ...
                                # Clamp to [0, 1] range
                                norm_value = max(0.0, min(1.0, norm_value))
                            result["data"][item_name]["timeseries"][field][ts_str] = norm_value
                        else:
                            result["data"][item_name]["timeseries"][field][ts_str] = values[i]
    ...
    def _format_timestamp(timestamp: datetime) -> str:
        """
        Format a timestamp for the raw format.
